...
def compute_avg_energy_by_category(data, qid_to_cat):
    """Compute average energy (Joules) per category.

    Returns (energy_dict, oom_counts_dict).
    """
    cat_energy = defaultdict(list)
    ...
    for dp in data:
        ...
        if dp.get("skipped") == "OOM":
            ...
            continue
        choice = dp["choices"][0]
        energy = choice.get("energy_joules")
        if energy is not None:
            cat_energy[cat].append(energy)

    return {cat: np.mean(vals) for cat, vals in cat_energy.items()}, dict(cat_oom)


def main():
    parser = argparse.ArgumentParser(description="Plot average energy per question per category for WikiLong")
    ...
    parser.add_argument("--output", type=str, default="wiki_long/avg_energy_per_category.png")
    ...
    for i, f in enumerate(spec_files):
        ...
        energy, oom_counts = compute_avg_energy_by_category(data, qid_to_cat)
        color = colors[i % len(colors)]

        if not energy:
            print(f"  {model_label}: no energy_joules data, skipping")
            continue
    ...
    ax.set_title("Average Energy per Question (WikiLong)")
    ax.set_xticks(x)
