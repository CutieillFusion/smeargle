# Source: https://github.com/huggingface/transformers/blob/v4.57.3/src/transformers/models/llama/modeling_llama.py
# Modifications are denoted by the symbol: [MODIFIED]
# - KVCacheAdapter wrapping for preallocated KV cache compatibility
# - Tree mask support for speculative decoding
# - Hidden states collection at specific layers (2, len//2, len-3)
...
class LlamaModel(LlamaPreTrainedModel):
    def __init__(self, config: LlamaConfig):
        ...
        # [MODIFIED] Tree mask support for speculative decoding
        self.tree_mask = None
        self.tree_mode = None
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        if _tree_active and causal_mask is not None:
            ...
            causal_mask[:, :, -tree_len:, -tree_len:][tree_mask == 0] = torch.finfo(causal_mask.dtype).min

        hidden_states = inputs_embeds
        position_embeddings = self.rotary_emb(hidden_states, position_ids)

        # [MODIFIED] Collect hidden states at specific layers for the draft model
        all_hidden_states = ()

        for i, decoder_layer in enumerate(self.layers[: self.config.num_hidden_layers]):
            if i == 2 or i == len(self.layers) // 2 or i == len(self.layers) - 3:
