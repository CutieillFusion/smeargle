"""QuAC grader — token-level F1 (model-card metric)."""
...
MODEL_CARD_TARGET = 44.9  # base 1-shot f1
...
def main():
    ...
    for qid, out in answers.items():
        ...
        f1_total += best_f1(pred, [ref] if ref else [""])
        n += 1
    score = f1_total / n if n else 0.0
    print_score("QuAC", "f1", score, n, target=MODEL_CARD_TARGET)
