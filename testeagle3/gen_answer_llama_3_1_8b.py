    ...
    for question in tqdm(questions):
        ...
        for i in range(num_choices):
            ...
            if use_eagle3:
                ...
                accuracy_per_position = [accept_length_per_position[j] / len(accept_lengths) for j in range(len(accept_length_per_position))]
                
                for accept_length in accept_lengths:
                    global_acceptance_lengths[accept_length - 1] += 1.0

                alpha_per_position = [accuracy_per_position[0]] + [alpha / accuracy_per_position[i] for i, alpha in enumerate(accuracy_per_position[1:])]
        ...
        if num_questions > 0:
            global_acceptance_rate = [x / num_questions for x in global_acceptance_lengths]
            
            plt.figure(figsize=(8,5))
            plt.plot(range(1, len(global_acceptance_rate) + 1), global_acceptance_rate, marker='o')
            plt.xlabel("Position")
            ...
            plt.savefig(os.path.join(chart_dir, "global_acceptance_rate_per_position.png"))
            plt.close()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--answer-file-path", 
        type=str, 
        help="The path to the output answer file."
    )
    parser.add_argument(
        "--max-new-token",
        type=int,
